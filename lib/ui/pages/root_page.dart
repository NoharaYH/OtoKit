      ...
      child: Stack(
        children: [
          ...
          RepaintBoundary(
            ...
            child: PageShell(
              backgroundOverride: Consumer2<GameProvider, MaiMusicProvider>(
                builder: (context, gp, maiMusicProvider, _) => AnimatedBuilder(
                  ...
                  builder: (context, _) {
                    if (gp.isThemeGlobal) {
                      ...
                      return AnimatedSwitcher(
                        ...
                        child: KeyedSubtree(
                          key: ValueKey(gp.resolvedTheme(baseSkin).themeId),
                          child: gp
                              .resolvedTheme(baseSkin)
                              .buildBackground(context),
                        ),
                      );
                    } else {
                      final double t = gp.pageValueNotifier.value.clamp(
                        0.0,
                        1.0,
                      );
                      final nav = context.read<NavigationProvider>();
                      final isUtage =
                          nav.currentTag == PageTag.musicData &&
                          maiMusicProvider.isUtageMode;

                      final AppTheme maiSkin;
                      if (isUtage) {
                        maiSkin = const UtageTheme();
                      } else {
                        final maiTheme = ThemeCatalog.findThemeById(
                          gp.maiSkinId,
                        );
                        maiSkin = gp.resolvedTheme(maiTheme);
                      }
                        ...
                        children: [
                          ...
                          if (t < 1.0)
                            AnimatedSwitcher(
                              duration: const Duration(milliseconds: 300),
                              child: KeyedSubtree(
                                key: ValueKey(maiSkin.themeId),
                                child: maiSkin.buildBackground(context),
                              ),
                            ),
              ...
              child: Stack(
                children: [
                  ...
                  // 3. 全局页眉操作区
                  Consumer2<GameProvider, MaiMusicProvider>(
                    builder: (context, gp, maiMusicProvider, _) =>
                        AnimatedBuilder(
                          animation: gp.pageValueNotifier,
                          builder: (context, _) {
                            ...
                            if (gp.isThemeGlobal) {
                              ...
                              final double page = gp.pageValueNotifier.value
                                  .clamp(0.0, 1.0);
                              final nav = context.read<NavigationProvider>();
                              final isUtage =
                                  nav.currentTag == PageTag.musicData &&
                            ...
                            return Positioned(
                              top:
                                  UiSizes.getTopMarginWithSafeArea(context) +
                                  12.0,
                              right:
                                  UiSizes.getHorizontalMargin(context) + 12.0,
                              child: Consumer<NavigationProvider>(
                                builder: (context, nav, child) {
                                  return Container(
